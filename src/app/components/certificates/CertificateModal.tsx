...
const BADGE_STYLES: Record<Badge['level'], { bg: string; border: string; label: string; stars: number }> = {
  bronze: { bg: '#FFF7ED', border: '#EA580C', label: 'Bronze', stars: 1 },
  silver: { bg: '#F8FAFC', border: '#94A3B8', label: 'Silver', stars: 2 },
  gold: { bg: '#FFFBEB', border: '#EAB308', label: 'Gold', stars: 3 },
};
...
export default function CertificateModal({ certificate, badge, onClose }: CertificateModalProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
        {/* Top bar */}
        <div
        ...
        >
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl flex items-center justify-center text-xl" style={{ backgroundColor: GOLD }}>
              <Award size={20} style={{ color: NAVY }} />
            </div>
            <div>
        ...
        <div className="p-6 lg:p-8">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center justify-center gap-3 mb-4">
                <span className="text-4xl"><Award size={20} style={{ color: NAVY }} /></span>
          ...
          >
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/70"><Award size={24} style={{ color: badgeStyle.border }} /></div>
            ...
            <div className="flex items-center gap-1">
              {[1, 2, 3].map((i) => (
                <Star
                  key={i}
                  size={16}
                  fill={i <= badgeStyle.stars ? badgeStyle.border : 'none'}
                  stroke={GOLD}
                />
              ))}
            </div>
